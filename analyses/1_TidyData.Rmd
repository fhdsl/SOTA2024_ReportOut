...
## Replace timestamp with just a date to obscure possibly identifying information

```{r}
resultsTidy %<>% mutate(TimestampID = as.integer(as.factor(Timestamp)),
...
resultsTidy %<>%
  ...
  mutate(group_col = factor(InstitutionalAffiliation,
                            levels = sample(unique(InstitutionalAffiliation))
                            )
        ) %>%
  group_by(group_col) %>%
  mutate(institutionID = factor(cur_group_id())) %>%
  ungroup()
```
...
## Create an additional user type stratification per reviewer request
...
resultsTidy %<>%
  mutate(
    ExpType = case_when(
      humanGenomicFlag == TRUE ~ "- More experienced",
