...
<details><summary>Code for making tables to look at the response distribution data</summary>
...
responseDf <- resultsTidy %>% 
  select(Timestamp, UserType, FurtherSimplifiedDegrees, institutionID) %>% 
  mutate(date = ymd(as_date(ymd_hms(Timestamp)))) %>% 
  group_by(date, UserType, FurtherSimplifiedDegrees, institutionID) %>% 
  summarize(count = n())
...
```{r}
paste0("Unique days of advertisement (at least): ", length(unique(socialMedia$date)))
```

</details>

See plot in above subsection for pattern
...
```{r}
highest_days <- responses_only %>% arrange(-daily_count) %>% head(2)

highest_days
```

```{r}
highest_days$date %in% socialMedia$date

socialMedia[which(socialMedia$date == highest_days$date[1] | socialMedia$date == highest_days$date[2]),]
```
...
```{r}
paste0("Average daily response count: ", mean(responses_only$daily_count)) #1.25
```
...
paste0("Most common daily response count: ", most_common) #0
paste0("Number of days with 0 reponses : ", sum(responses_only$daily_count == most_common))#21 days with 0 responses
...
> followedby 1 (8 different days) if restricted to non-zero responses
...
paste0("Most common daily response count (ignoring days without responses): ", most_common_withoutZeros) #1
paste0("Number of days with ", most_common_withoutZeros, " responses: ", sum(responses_only$daily_count == most_common_withoutZeros)) #8 days with 1 response
```
...
The raw institutional affiliation was removed from the tidy data since it could be identifying, however, we used an institutional ID in place of it that we can check. 

<details><summary> Code making a plot to support the above statement </summary>

```{r}
p3 <- toPlot %>%
  ungroup() %>%
  select(date, institutionID, count) %>%
  group_by(date, institutionID) %>%
  summarize(daily_inst_count = sum(count)) %>%
  drop_na() %>%
  ggplot(aes(x = date, y = daily_inst_count, fill = institutionID)) +
  geom_bar(stat="identity", color = 'white')

p3 <- plot_aesthetics(p3, ylabel = "Institutional Responses", bar = TRUE, xticks = FALSE) + scale_fill_viridis_d()
```

</details>

```{r}
p3 / p2 + plot_layout(heights = c(3,1))
```

While visual support for this statement is in the plot, it may be difficult to distinguish sometimes, so if we try to filter a daily institutional count to days with greater than 2 responses from the same institutional ID, no rows are retained
...
<details><summary>Code to make a plot to support the above statement</summary>
...
p4 <- plot_aesthetics(p4, ylabel = "Usertype Responses", legend = TRUE, bar = TRUE, xticks = FALSE)
...
```{r}
p4 / p2 + plot_layout(heights = c(3,1))
```
