...
### Comma separated columns (making one hot encoded output)
...
vectorized_datatypes <- vectorize_comma(pca_data_ar, "TypesOfData") %>%
  select(!c("used in training for analysis of genomes (variant calling)", "i don't analyze data on anvil", "i store data in anvil. i don’t analyze it.")) #remove the ones provided through "Other submission"
```
...
pca.var <- factoextra::get_pca_var(data.pca)
ggcorrplot::ggcorrplot(pca.var$cos2[, 1:4]) + 
  theme(legend.position = "none",
        axis.text.y = element_text(size = 7),
        plot.margin = margin(0,5,0,25, unit = "mm")) +
  coord_flip()
```
