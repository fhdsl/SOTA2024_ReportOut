...
demoPlotRaw <- resultsTidy %>%
  ggplot(aes(y=fct_rev(fct_infreq(AnVILDemo)),
             fill = UserType)) +
  geom_bar() +
...
forumdf <- resultsTidy %>%
  ...
  # Create additional stratification per reviewer request
  mutate(
    ExpType = case_when(
      HumanGenomicExperience %in% c("Extremely experienced", "Moderately experienced") ~ "- More experienced",
      TRUE ~ "- Less experienced"
    )
  ) %>%
  mutate(UserTypeStratified = paste(UserType, ExpType))
```

#### Raw responses

```{r fig6c, fig.width = 9.7, fig.height=6}
forumPlotBody <- function(ggplotItem) {
  return(
    ggplotItem +
      geom_bar() +
...
forumPlotRaw <- 
  forumPlotBody(ggplot(forumdf, aes(
