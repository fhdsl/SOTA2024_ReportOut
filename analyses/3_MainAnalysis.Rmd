...
First, we use a mutate statement to better fit the detailed descriptions on the plot. We then send this data to ggplot which will count the categories for the x-axis, and the usage descriptions on the y-axis (ordered by count so highest count is on the top). We fill with the `UserType` description we've assigned. We make edits for the theme and labels and finally add a geom_text label for the count next to the bars before we save the plot.

</details>

```{r fig2a, fig.height = 5.5, fig.width = 8.5}
typeOfUserPlot <- resultsTidy %>%
  ...
  ggplot(aes(y = fct_rev(fct_infreq(CurrentUsageDescription)),
             fill = UserType)) +
  geom_bar() +
...
ggsave(here("resources/plots/respondent_usagedescription.png"), plot = typeOfUserPlot, width = 8.5, height = 5.5, units = "in")
```
...
First we send this data to ggplot and make a bar chart with the y-axis representing the degrees (ordering by the count number such that higher counts are first (and the sum) because otherwise the 2 MDs are located after the high school and master's in progress bars (1 each)). The x-axis represents the count, and the fill is used to specify user type (returning or potential AnVIL users). We use a stacked bar chart and include labels above each bar of the total sum for that degree type.
...
```{r fig2b, fig.height = 5.5, fig.width = 8.75}
...
ggsave(here("resources/plots/degree_furthersimplified_usertype.png"), plot = degreePlot, width = 8.75, height = 5.5, units = "in")
```
...
ggsave(here("resources/plots/kind_of_work_heatmap.png"), plot = kowPlot, height = 4, width = 7.8, units = "in")
```
...
First, we set the factor level for the further simplified institutional type column (`FurtherSimplifiedInstitutionalType`) so that we know the order on the y-axis when plotting. We plot this as a bar plot with the institutional type on the y-axis and the count on the x-axis and fill the stacked bars according to `UserType`. We add text labels to the bars displaying the sum of the institutional type. We also use custom annotation grobs that break down which institutional types are part of each further simplified institutional type (as defined in `TidyData.Rmd`). Note the liberal uses of spaces to try to align these sub-labels. Finally, we pass the plot to the shared function `stylize_bar()` to change axis labels, fill colors, etc.     

</details>

```{r fig1c, fig.height = 5.4, fig.width = 8.8}
...
ggsave(here("resources/plots/institutionalType_simplified_allResponses_colorUserType.png"), plot = instTypePlot, height = 5.4 , width = 8.8 , units = "in")
```
...
```{r fig7a, fig.width = 9, fig.height = 6}
...
ggsave(here("resources/plots/tooldataresourcecomfortscore_singlepanel.png"), plot = PlotToolKnowledge_avg_score, width = 9, height = 6, units = "in")
```

### Prepare and plot the data, usertype and experience stratified

```{r}
toPlotToolKnowledgeStrat <- data.frame()

for(strat in unique(resultsTidy$UserTypeStratified)){
  toPlotToolKnowledgeStrat <- rbind(
    toPlotToolKnowledgeStrat,
    resultsTidy %>%
      filter(UserTypeStratified == strat) %>%
      select(starts_with("Score_")) %>%
      colSums() %>%
      as.data.frame() %>% `colnames<-`(c("totalScore")) %>%
      mutate(nscores = sum(resultsTidy$UserTypeStratified == strat),
             avgScore = totalScore / nscores,
             UserType = strat) %>%
      mutate(WhereTool = rownames(.)) %>%
      separate(WhereTool, c("AnVILorNo", "Tool"), sep = "Tech", remove = TRUE) %>%
      mutate(AnVILorNo =
              ...
              Tool =
                recode(Tool, "JupyterNotebooks" = "Jupyter Notebooks",
                             ...
                             "Terra" = "Terra Workspaces",
                             "BioconductorRStudio" = "Bioconductor & RStudio"))
    ) %>% mutate(UserType = factor(str_replace(UserType, " - ", "\n"),
                                          levels = c("Potential User\nLess experienced",
                                                     "Potential User\nMore experienced",
                                                     "Returning User\nLess experienced",
                                                     "Returning User\nMore experienced")))
}
...
```{r}
roi <- toPlotToolKnowledgeStrat[which(toPlotToolKnowledgeStrat$Tool == "Bioconductor & RStudio"),]
toPlotToolKnowledgeStratSeparateBR <- rows_append(toPlotToolKnowledgeStrat, data.frame(
          UserType = rep(roi$UserType,2),
          avgScore = rep(roi$avgScore,2),
          AnVILorNo = rep(roi$AnVILorNo,2),
          Tool = c("Bioconductor", "RStudio")
...
PlotToolKnowledgeStrat_avg_score <-
  ggplot(toPlotToolKnowledgeStratSeparateBR, aes(y = reorder(Tool, avgScore), x = avgScore)) +
  geom_point(aes(color = UserType, shape = AnVILorNo), size = 3.5)


PlotToolKnowledgeStrat_avg_score %<>% PlotToolKnowledge_customization(usertypeStratified = TRUE)

PlotToolKnowledgeStrat_avg_score <- PlotToolKnowledgeStrat_avg_score +
...
ggsave(here("resources/plots/tooldataresourcecomfortscore_strat_singlepanel.png"), plot = PlotToolKnowledgeStrat_avg_score, width = 9, height = 6, units = "in")
```
...
```{r, fig.width = 11.2, fig.height = 5.7}
everyone_type_data <- plot_type_data(typeOfDataDf)

everyone_type_data

ggsave(here("resources/plots/typesOfData.png"), plot=everyone_type_data, width = 11.2, height = 5.7, units ="in")
```
...
ggsave(here("resources/plots/typesOfData_clinical.png"), plot=clinical_type_data, width = 11.2, height = 5.7, units ="in")
```
...
ggsave(here("resources/plots/typesOfData_humangenomic.png"), plot=humangenomic_type_data, width = 11.2, height = 5.7, units ="in")
```
...
ggsave(here("resources/plots/researchExperienceLevel_sequentialColor_noUserTypeSplit.png"), plot = genomicsExpPlot, width = 11.2, height = 5.7, units ="in")
```
...
genomicsExpPlot_n <-
  ggplot(experienceDf_pct, aes(x=n, y = UserType, fill=experienceLevel)) +
  facet_wrap(~researchType, ncol=1) +
  geom_bar(stat = "identity", position = "stack")

genomicsExpPlot_n %<>% stylize_bar(xlabel = "Number of\nRespondents", usertypeColor = FALSE, sequentialColor = TRUE, labelBars = FALSE)

genomicsExpPlot_n <- genomicsExpPlot_n +
...
genomicsExpPlot_replot <-
  ...
  facet_wrap(~researchType, ncol=1, strip.position = "right", labeller = label_wrap_gen(width = 12)) +
  geom_bar(stat = "identity", position = "stack")

genomicsExpPlot_replot %<>% stylize_bar(xlabel = "Percent of\nRespondents", usertypeColor = FALSE, sequentialColor = TRUE, labelBars = FALSE)
...
ggsave(here("resources/plots/experience_data_cats.png"), plot = genomicsExpPlot_replot, width = 9, height = 4, units = "in")
```

```{r fig3b, fig.height = 5.7, fig.width = 7.2}
...
ggsave(here("resources/plots/experience_data_cats_venn.png"), plot = vennExperience, width = 7.2, height = 5.7, units = "in")
```
...
ggsave(here("resources/plots/controlled_access_datasets.png"), plot = dataInterestPlot, width = 9, height = 6, units = "in")
...
ggsave(here("resources/plots/whichcontrolleddata.png"), plot = everyoneDataPlot, width = 11.2, height = 5.7, units ="in")
```
...
ggsave(here("resources/plots/whichcontrolleddata_clinical.png"), plot = clinicalDataPlot, width = 11.2, height = 5.7, units ="in")
```
...
ggsave(here("resources/plots/whichcontrolleddata_humangenomic.png"), plot = humanGenomicDataPlot, width = 11.2, height = 5.7, units ="in")
```
...
ggsave(here("resources/plots/whichcontrolleddata_nonhumangenomic.png"), plot = nonHumanGenomicDataPlot, width = 11.2, height = 5.7, units ="in")
```
...
```{r fig6a, fig.width = 9, fig.height = 6}
demoPlotRaw <- supportPlotBody(ggplot(resultsTidy,
  (aes(y=fct_rev(fct_infreq(AnVILDemo)),
             fill = UserType))), demo = TRUE)

demoPlotRaw %<>% stylize_bar(labelBars = FALSE)

demoPlotRaw

ggsave(here("resources/plots/demo_raw_responses.png"), plot = demoPlotRaw, width = 9, height = 6, units = "in")
```  

#### Raw responses, stratified

```{r fig.width = 9, fig.height = 6}
demoPlatRawStrat <- supportPlotBody(ggplot(resultsTidy,
  (aes(y=fct_rev(fct_infreq(AnVILDemo)),
             fill = UserTypeStratified))), demo = TRUE)

demoPlatRawStrat %<>% stylize_bar(labelBars = FALSE, usertypeColor = FALSE, usertypeStratified = TRUE)

demoPlatRawStrat

ggsave(here("resources/plots/demo_raw_stratified_responses.png"), plot = demoPlatRawStrat, width = 9, height = 6, units = "in")
```

#### Responses recoded to focus on awareness

```{r fig6b, fig.width = 9, fig.height = 6}
demoPlot <- supportPlotBody(ggplot(resultsTidy,
  (aes(y=fct_rev(fct_infreq(AnVILDemoAwareness)),
             fill = UserType))), demo = TRUE)
...
ggsave(here("resources/plots/demo_awareness_responses.png"), plot = demoPlot, width = 9, height = 6, units = "in")
```

#### Recoded responses, stratified

```{r fig.width = 9, fig.height = 6}
demoPlotStrat <- supportPlotBody(ggplot(resultsTidy,
  (aes(y=fct_rev(fct_infreq(AnVILDemoAwareness)),
             fill = UserTypeStratified))), demo = TRUE)

demoPlotStrat %<>% stylize_bar(ylabel = "Awareness", labelBars = FALSE, usertypeColor = FALSE, usertypeStratified = TRUE)

demoPlotStrat

ggsave(here("resources/plots/demo_awareness_stratified_responses.png"), plot = demoPlotStrat, width = 9, height = 6, units = "in")
```
...
```{r}
forumdf_recode <- pivot_wider(forumdf,
    id_cols = c(TimestampID, UserType, ExpType, UserTypeStratified),
    values_from = forumAwareness,
    names_from = AnVILSupportForum,
    names_prefix = "forumAwareness_") %>%
  mutate(forumAwareness = coalesce(!!!select(., starts_with("forumAwareness_")))) %>%
  select(!starts_with("forumAwareness_"))
```

#### Raw responses

```{r fig6c, fig.width = 9.7, fig.height=6}
forumPlotRaw <-
  supportPlotBody(ggplot(forumdf, aes(
    y = fct_rev(fct_infreq(AnVILSupportForum)), fill = UserType
  )), forum = TRUE)
...
ggsave(here("resources/plots/forum_raw_responses.png"), plot = forumPlotRaw, width = 9.7, height = 6, units = "in")
...
forumPlotRawStrat <-
  supportPlotBody(ggplot(forumdf, aes(
    y = fct_rev(fct_infreq(AnVILSupportForum)), fill = UserTypeStratified
  )), forum = TRUE)

forumPlotRawStrat %<>% stylize_bar(labelBars = FALSE, usertypeColor = FALSE, usertypeStratified = TRUE)

forumPlotRawStrat

ggsave(here("resources/plots/forum_raw_strat_responses.png"), plot = forumPlotRawStrat, width = 9.7, height = 6, units = "in")

```

#### Responses recoded to focus on awareness

```{r fig6d, fig.width = 9, fig.height = 6}
forumPlot <- supportPlotBody(ggplot(forumdf_recode, aes(
  y = fct_rev(fct_infreq(forumAwareness)), fill = UserType
)), forum = TRUE)

forumPlot %<>% stylize_bar(ylabel = "Awareness", labelBars = FALSE, usertypeStratified = FALSE)

forumPlot

ggsave(here("resources/plots/forum_awareness_responses.png"), plot = forumPlot, width = 9, height = 6, units = "in")
```

#### Awareness responses, stratified

```{r fig.width = 9, fig.height = 6}
forumPlotStrat <- supportPlotBody(ggplot(forumdf_recode, aes(
    y = fct_rev(fct_infreq(forumAwareness)), fill = UserTypeStratified
)), forum = TRUE)

forumPlotStrat %<>% stylize_bar(ylabel = "Awareness", labelBars = FALSE, usertypeColor = FALSE, usertypeStratified = TRUE)

forumPlotStrat

ggsave(here("resources/plots/forum_awareness_strat_responses.png"), plot = forumPlotStrat, width = 9, height = 6, units = "in")
```
...
### Prepare and plot the data, usertype stratification
...
```{r fig5a, fig.height = 5.7, fig.width = 10.5}
...
ggsave(here("resources/plots/dumbbellplot_xlim16_revaxis_rankfeatures.png"), plot = gdumbbell, height = 5.7, width = 10.5, units = "in")

```

### Prepare and plot the data, usertype and experience level stratification

```{r}
totalRanksdfStrat <- data.frame()

for(strat in unique(resultsTidy$UserTypeStratified)){
  totalRanksdfStrat <- rbind(totalRanksdfStrat,
    resultsTidy %>%
      filter(UserTypeStratified == strat) %>%
      select(starts_with("ReturningRank")| starts_with("PotentialRank")) %>%
      colSums(na.rm = TRUE) %>%
      as.data.frame() %>% `colnames<-`(c("totalRank")) %>%
      mutate(nranks = sum(resultsTidy$UserTypeStratified == strat),
             avgRank = totalRank / nranks,
             UserType = strat) %>%
      mutate(UsertypeFeature = rownames(.)) %>%
      separate(UsertypeFeature, c("temp", "Feature"), sep = "Rank", remove = TRUE) %>%
      mutate(Feature =
                      case_when(Feature == "EasyBillingSetup" ~ "Easy billing setup",
                                Feature == "FlatRateBilling" ~ "Flat-rate billing rather than use-based",
                                Feature == "FreeVersion" ~ "Free version with limited compute or storage",
...
totalRanksdfStrat %<>% mutate(UserType = factor(str_replace(UserType, " - ", "\n"),
                                      levels = c("Potential User\nLess experienced",
                                                 "Potential User\nMore experienced",
                                                 "Returning User\nLess experienced",
                                                 "Returning User\nMore experienced"))) %>%
...
```{r fig.width = 8.4, fig.height = 5.7}
gdumbbellStrat <- ggplot(totalRanksdfStrat, aes(x = avgRank, y = reorder(Feature, -avgRank))) +
    geom_line() +
    geom_point(aes(color = UserType), size = 3.5) +
    ggtitle("Rank the following features according to\ntheir importance to you as a potential user\nor for your continued use of the AnVIL.")


gdumbbellStrat %<>% stylize_dumbbell(preference = TRUE, xmax = 6, usertype = FALSE, usertypeStratified = TRUE)

gdumbbellStrat <- gdumbbellStrat +
  ...
  annotation_custom(textGrob("Least\npreferred",
                             gp=gpar(fontsize=8, fontface = "bold")),
                             xmin=6,xmax=6,ymin=-0.1,ymax=-0.1)

ggsave(here("resources/plots/dumbbellplot_xlim16_revaxis_strat_rankfeatures.png"), plot = gdumbbellStrat, height = 5.7, width = 10.5, units = "in")
```
...
```{r fig5b, fig.width = 8.4, fig.height = 5.7}
...
tdumbbell +
  annotation_custom(textGrob("Most\npreferred", gp=gpar(fontsize=8, fontface = "bold")),xmin=-1,xmax=-1,ymin=-0.25,ymax=-0.25) +
  annotation_custom(textGrob("Least\npreferred", gp=gpar(fontsize=8, fontface= "bold")),xmin=-5,xmax=-5,ymin=-0.25,ymax=-0.25)

ggsave(here("resources/plots/dumbbellplot_xlim15_revaxis_trainingmodalitypref.png"), plot = tdumbbell, height = 5.7, width = 8.4, units = "in")
```

### Prepare and plot the data, usertype and experience level stratification

```{r}
toPlotTrainingRanksStrat <- data.frame()

for(strat in unique(resultsTidy$UserTypeStratified)){
  toPlotTrainingRanksStrat <- rbind(toPlotTrainingRanksStrat,
    resultsTidy %>%
      filter(UserTypeStratified == strat) %>%
      select(starts_with("AnVILTrainingWorkshops")) %>%
      colSums(na.rm = TRUE) %>%
...
toPlotTrainingRanksStrat %<>% mutate(TrainingType = recode(TrainingType,
   "SpecEvent" = "AnVIL-specific event",
   "OnSite" = "On-site at my institution",
   "Conference" = "Conference (e.g., CSHL, AMIA)")) %>%
   mutate(UserType = factor(str_replace(UserType, " - ", "\n"), levels = c("Potential User\nLess experienced",
                                                 "Potential User\nMore experienced",
                                                 "Returning User\nLess experienced",
                                                 "Returning User\nMore experienced")))
...
```{r fig.width = 8.4, fig.height = 5.7}
tdumbbellStrat <- ggplot(toPlotTrainingRanksStrat, aes(x = avgRank, y = reorder(TrainingType, -avgRank))) +
    geom_line() +
    geom_point(aes(color = UserType), size = 3.5) +
    ggtitle("Please rank how/where you would prefer\nto attend AnVIL training workshops.")


tdumbbellStrat %<>% stylize_dumbbell(preference = TRUE, xmax = 5, usertype = FALSE, usertypeStratified = TRUE)

tdumbbellStrat <- tdumbbellStrat +
  theme(legend.position = "bottom") +
  annotation_custom(textGrob("Most\npreferred",
                             gp=gpar(fontsize=8, fontface = "bold")),
                             xmin=1,xmax=1,ymin=-0.1,ymax=-0.1) +
  annotation_custom(textGrob("Least\npreferred",
                             gp=gpar(fontsize=8, fontface = "bold")),
                             xmin=5,xmax=5,ymin=-0.1,ymax=-0.1)

ggsave(here("resources/plots/dumbbellplot_xlim15_revaxis_strat_trainingmodalitypref.png"), plot = tdumbbellStrat, height = 5.7, width = 8.4, units = "in")
```
...
```{r fig7b}
whereRunPlot <-
  supportPlotBody(ggplot(whereRundf, aes(
      y = fct_rev(fct_infreq(WhereAnalysesRun)), fill = UserType
  )), platform = TRUE)
...
ggsave(here("resources/plots/where_run.png"), plot = whereRunPlot, width = 9.8, height = 5.7, units = "in")

```

#### Additional usertype stratification

```{r}

whereRundf %<>%
  mutate(UserType = factor(str_replace(UserTypeStratified, " - ", "\n"),
                                   levels = c("Potential User\nLess experienced",
                                              "Potential User\nMore experienced",
                                              "Returning User\nLess experienced",
                                              "Returning User\nMore experienced")))

whereRunPlotStrat <-  supportPlotBody(ggplot(whereRundf, aes(
      y = fct_rev(fct_infreq(WhereAnalysesRun)), fill = UserTypeStratified
    )), platform = TRUE)

whereRunPlotStrat %<>% stylize_bar(ylabel = "Platform", labelBars = FALSE, usertypeColor = FALSE, usertypeStratified = TRUE)

whereRunPlotStrat

ggsave(here("resources/plots/where_run_stratified.png"), plot = whereRunPlotStrat, width = 9.8, height = 5.7, units = "in")

```

## Preferences: DMS compliance/data repositories

```{r, fig.width = 9.2, fig.height=7.2}
...
dmsPlot <- resultsTidy %>%
  ...
  ggplot(aes(
             y = fct_rev(fct_infreq(RepositoriesMut)),
             fill = UserType)) +
  geom_bar() +
...
ggsave(here("resources/plots/DMS_Repositories.png"), plot = dmsPlot, width = 9.2, height = 7.2, units = "in")
```
...
```{r, fig.width = 9.8, fig.height = 5.7}

fundingSourcePlot <- toPlotFundingSource %>%
  ggplot(aes(y = fct_rev(fct_infreq(FundingSources)),
             fill = UserType)) +
  geom_bar() +
...
ggsave(here("resources/plots/fundingsources.png"), plot = fundingSourcePlot, width = 9.8, height = 5.7, units = "in")
```
...
compNeedsPlot <- resultsTidy %>%
  ...
  separate_longer_delim(NeededResources,
           delim = ", ") %>%
  ggplot(aes(
             y = fct_rev(fct_infreq(NeededResources)),
             fill = "#25445A")) +
  geom_bar() +
  geom_text(aes(label = after_stat(..count..)), stat = "count",
                 position = "stack", hjust = -1, size=4) +
