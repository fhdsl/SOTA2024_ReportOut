...
author: "Kate Isaac and Ava Hoffman"
...
ggsave(here("figures/user_identification_panels.png"), width=12, height=16, units = "in")
```

## Figure 3: User research experience  

```{r, fig.width=9, fig.height = 11}
a_sub <- wrap_elements((genomicsExpPlot_n + theme(
  strip.text = element_blank(),
  strip.background = element_blank()
)) + (genomicsExpPlot_replot + theme(axis.text.y = element_blank())) + plot_layout(guides = "collect"))
...
a_sub /
  ...
  plot_annotation(title = "User research experience",
                  ...
                  theme = theme(plot.title = element_text(size = 18),
                                plot.tag = element_text(face = "bold"))) +
  plot_layout(widths = c(14), heights = c(10, 14))

ggsave(here("figures/user_research_experience_panels.png"), width=9, height=11, units = "in")
...
kowpersona <- full_join(
  tidy_results_withPersona %>%
    select(persona, TimestampID) %>%
    mutate(TimestampID = factor(TimestampID),
           persona = na_if(persona, "None assigned")),
  dfForPlotKOW, by = "TimestampID") %>%
  group_by(UserType, persona, TimestampID) %>%
...
ggsave(here("figures/personas_panels.png"), width=7, height=7, units="in")
```
...
ggsave(here("figures/user_preferences_panels.png"), width=10, height=12, units="in")
```
...
ggsave(here("figures/awareness_panels.png"), width=16, height=10, units="in")
```
...
ggsave(here("figures/user_tech_comfort_panels.png"), width=10, height=12, units="in")
```
